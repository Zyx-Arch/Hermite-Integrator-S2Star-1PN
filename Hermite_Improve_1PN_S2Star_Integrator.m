...
%计算并运行积分器
[t0,s0,step_history0]=hermite_integrator(t_span,yuanchu,GM_sgrA,c_AU_year,opts,e_val);
...
for idx = 2:length(e_array)
    ...
    %计算并运行积分器
    tic;
    [t1,s1,step_history]=hermite_integrator(t_span,tuoyuanchu,GM_sgrA,c_AU_year,opts,e_val);
    ...
    %输出结果
    fprintf(['偏心率e=%.4f\n' ...
        '远心点=%.6fAU,近心点=%.6fAU,远点相对误差=%.6f%%,近点相对误差=%.10f%%,近与远误差比值=%.6f\n' ...
